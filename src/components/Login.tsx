...
export default function Login({ setAuth }: { setAuth: (val: boolean) => void }) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Simple demo validation: accepts any password for now
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="relative group">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 text-zinc-900 dark:text-white focus:ring-0 focus:border-purple-500 dark:focus:border-purple-500 outline-none transition-all duration-300 font-medium"
              required
            />
          </div>
          ...
          >
            Access EdgePDF <ArrowRight className="w-6 h-6" />
          </button>
        </form>
      </div>
    </div>
